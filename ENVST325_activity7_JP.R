...
mod.improved <- lm(log.ch4 ~ airTemp+
                 ...
                 log.DIP+
                 log.precip+ BorealV+
                 Residence.Time..days., data=ghg)
summary(mod.improved)

res.improved <- rstandard(mod.improved)
fit.improved <- fitted.values(mod.improved)

# qq plot
qqnorm(res.improved, pch=19, col="grey50")
qqline(res.improved)

# shapiro-wilks test
shapiro.test(res.improved)

plot(fit.improved,res.improved, pch=19, col="grey50")
abline(h=0)

# Question #1 / #2

mod.co2_trans <- lm(co2_trans ~ airTemp+
                     log.age+
                     mean.depth+
                     log.DIP+
                     log.precip+
                     BorealV+
                     runoff, data=ghg)
summary(mod.co2_trans)

# Stepwise selection
model_final <- ols_step_both_p(mod.co2_trans)

# Final chosen model manually rebuild
mod.final <- lm(co2_trans ~ airTemp +
                  mean.depth +
                  log.age +
                  log.DIP +
                  BorealV +
...
res.final <- rstandard(mod.final)
fit.final <- fitted.values(mod.final)

# QQ plot
qqnorm(res.final, pch=19, col="grey50")
qqline(res.final)

# Residual plot
plot(fit.final,res.final, pch=19, col="grey50")
abline(h=0)
...
crop_ts <- ts(crop_data$ET.in, # data
                start = c(2016,1), #start year 2016, month 1
                #first number is unit of time and second is observations within a unit
                frequency= 12) # frequency of observations in a unit
...
pist_ts <- ts(pist$ET.in, # data
              start = c(2016,1), #start year 2016, month 1
              #first number is unit of time and second is observations within a unit
              frequency= 12) # frequency of observations in a unit

acf(na.omit(pist_ts), # remove missing data
    lag.max = 24) # look at 2 years (24 months)

pacf.plot <- pacf(na.omit(pist_ts))

pist_y <- na.omit(pist_ts)
mod_pist <- arima(pist_y , # data 
                order = c(4,0,0))

newPist <- forecast(mod_pist)
newPist
...
fall_ts <- ts(fall$ET.in, # data
              start = c(2016,1), #start year 2016, month 1
              #first number is unit of time and second is observations within a unit
              frequency= 12) # frequency of observations in a unit

acf(na.omit(fall_ts), # remove missing data
    lag.max = 24) # look at 2 years (24 months)

pacf.plot <- pacf(na.omit(fall_ts))

fall_y <- na.omit(fall_ts)
mod_fall <- arima(fall_y , # data 
                  order = c(2,0,0))

newFall <- forecast(mod_fall)
newFall

plot(forecast(mod_pist,h=24), main="Pistachio Forecast")
plot(forecast(mod_fall,h=24), main="Fallow Forecast")
